...
class SeldDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        event_frame_logit = F.leaky_relu_(self.event_fc_1(self.event_dropout_1(x)))  # (batch_size, time_steps, n_classes)
        event_frame_logit = self.event_fc_2(self.event_dropout_2(event_frame_logit))

        # DOA: regression
        x_output = F.leaky_relu_(self.x_fc_1(self.x_dropout_1(x)))
        x_output = torch.tanh(self.x_fc_2(self.x_dropout_2(x_output)))
        y_output = F.leaky_relu_(self.y_fc_1(self.y_dropout_1(x)))
        y_output = torch.tanh(self.y_fc_2(self.y_dropout_2(y_output)))
        z_output = F.leaky_relu_(self.z_fc_1(self.z_dropout_1(x)))
